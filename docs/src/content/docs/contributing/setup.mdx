...
description: A guide to setting up your development environment for Peptide on Android.
template: doc
---

This page contains the guidelines for setting up your development environment for Peptide on Android. These guidelines are important to ensure that your development environment is set up correctly and you can start contributing to the project.
...
4.  Clone the repository.
    ...
    ```sh
    git clone --recursive https://github.com/peptidechat/android.git
    ```
...
8.  Copy the `peptidebuild.properties.example` file to `peptidebuild.properties` and fill in the required values.

    ```sh
    cp peptidebuild.properties.example peptidebuild.properties
    ```

    The `peptidebuild.properties` file contains the following properties:
...
11. **You're all set!** You can now start contributing to Peptide on Android.
